...
function solution(scores) {
    ...
    for (let i = 0; i < scores.length; i++) {
        ...
        var largeCount = temp.filter(e => e >= temp[i]).length;
        var smallCount = temp.filter(e => e <= temp[i]).length;
        
        if (largeCount <= 1 || smallCount <= 1) {
            temp.splice(i, 1);
        }
        
        var avg = temp.reduce((prev, curr) => prev + curr)/temp.length;
